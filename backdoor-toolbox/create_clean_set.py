...
parser.add_argument('-dataset', type=str, required=False, default='ISIC2019',
                    choices=default_args.parser_choices['dataset'])
parser.add_argument('-clean_budget', type=int, default=200)
...
def main():
    ...
    if args.dataset == 'gtsrb':
        ...
        clean_set = datasets.ImageFolder(os.path.join(os.path.join(data_dir, 'OCT/OCT2017_'), 'train'),
                                         data_transform)
        
        img_size = 256  # Standard size for OCT images
        ...
        clean_set = datasets.ImageFolder(os.path.join(os.path.join(data_dir, 'chest_xray'), 'train'),
                                         data_transform)
        ...
        clean_set = datasets.ImageFolder(os.path.join(data_dir, 'ISIC2019'),
                                         data_transform)
    ...
    if args.dataset != 'ember' and args.dataset != 'imagenet':
        ...
        testInd = args.clean_budget+args.clean_budget
        random.shuffle(id_set)
        clean_split_indices = id_set[:args.clean_budget]
        test_indices = id_set[args.clean_budget:testInd]
