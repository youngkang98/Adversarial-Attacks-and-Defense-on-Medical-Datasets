...
def get_alert_mask(args, original_output, shadow_output, unlearned_output, threshold_params=None, return_score=False):
    ...
    elif args.dataset == 'OCT' or args.dataset =='CXRAY' or args.dataset =='ISIC2019':
        if threshold_params is not None:
            # Using the same pattern as CIFAR10/ImageNet
            alert_mask = eval(args.metric_name) > threshold_params[f'threshold_{args.metric_name}']
            if args.hard_filter:
                hard_mask = torch.logical_and(unlearned_output[i, original_pred[i]] > 0.98, shadow_output[i, original_pred[i]] < 0.5)
                alert_mask = torch.logical_or(alert_mask, hard_mask)
            ...
            # Default threshold for OCT if no threshold_params provided
            alert_mask = u_s_diff > 0.15  # You can adjust this threshold value
...
def unlearn(args, params, clean_set_loader, test_set_loader, poison_transform):
    ...
    if (args.model_path.endswith('.pth.tar') or args.model_path.endswith('.pth')):
...
#     # Save the original stdout at the start of finetune
#     original_stdout = sys.stdout
    
#     # If logging is enabled, set up the Tee redirection
#     if args.log:
#         out_path = 'logs'
...
#         # Open the log file
#         log_file = open(out_path, 'a')  # Note: using 'a' for append mode
