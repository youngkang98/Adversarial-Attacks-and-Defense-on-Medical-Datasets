...
class BaDExpert(BackdoorDefense):
    ...
    def __init__(self, args, metric_name = 'triangle', defense_fpr=None):
        ...
        if args.dataset == 'cifar10' or args.dataset == 'gtsrb':
            ...
            clean_set_loader = torch.utils.data.DataLoader(
                clean_set,
                batch_size=params['batch_size'], shuffle=True, **params['kwargs'])
            test_set_loader = get_dataloader(args, False,trainOrTestData='Test')
        elif args.dataset == 'CXRAY':
            poison_transform = supervisor.get_poison_transform(
                poison_type='WaNet',
                dataset_name='CXRAY',
                target_class=config.target_class['CXRAY'],
                trigger_transform=data_transform,
                is_normalized_input=True,
                alpha=args.alpha if args.test_alpha is None else args.test_alpha,
                trigger_name=args.trigger,
            ...
            poison_transform = supervisor.get_poison_transform(
                ...
                target_class=config.target_class['ISIC2019'],
                trigger_transform=data_transform,
                is_normalized_input=True,
                alpha=args.alpha if args.test_alpha is None else args.test_alpha,
                trigger_name=args.trigger,
    ...
    def detect(self):
        ...
        # Load models
        original_model = supervisor.get_arch(args)
        shadow_model = supervisor.get_arch(args)
        unlearned_model = supervisor.get_arch(args)
