...
from art.estimators.classification import PyTorchClassifier,TFClassifier
...
def load_data(datapath, trainfile, testfile):
    if os.path.exists(datapath + '/x_test.npy'):
        ...
        for data_type in ['test']:
            ...
            for i in tqdm(range(len(datafile))):
                ...
                img_path = os.path.join(datapath, data_type, line[1])
    ...
    return (x_test, y_test), (mean_l2_train, mean_inf_train)
...
def set_up(args):
    ...
    # Ensure the shape of label_tensor is defined
    try:
        label_tensor = graph.get_tensor_by_name("Placeholder:0")
        label_shape = label_tensor.shape.as_list()
    except ValueError:
        # Define the number of classes (you need to set the actual number of classes)
        number_of_classes = 3  # Example: replace with the actual number of classes
