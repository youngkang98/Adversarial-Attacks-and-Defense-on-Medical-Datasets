...
def main():
    ...
    class OCTDataset(Dataset):
        ...
        def __getitem__(self, idx):
            # Get label first since we need it for the path
            label_str = self.df.iloc[idx]['label']
            # Construct path including the class directory
            img_path = os.path.join(self.data_path, label_str, self.df.iloc[idx]['image'])
            
            image = Image.open(img_path).convert('RGB')  # Convert to RGB to ensure 3 channels
            
            # Convert string label to numeric using label_map
    ...
    if args.poison_type == 'dynamic':
        ...
        if args.dataset == 'gtsrb':
            ...
            # train_data_path = os.path.join(data_dir, 'OCT/OCT2017_/train')
            # train_csv = os.path.join(data_dir, str(config.get_data_path('OCT2017-train-adv-0.1.csv')))
            
            # train_set = OCTDataset(
            #     data_path=train_data_path,
            #     csv_file=train_csv,
            #     transform=data_transform
            # )
            train_set = datasets.ImageFolder(os.path.join(os.path.join(data_dir, 'OCT/OCT2017_'), 'train'),
                                             data_transform)
            
            img_size = 224  # Standard size for OCT images
            num_classes = 4  # OCT typically has 4 classes: CNV, DME, DRUSEN, and NORMAL
        elif args.dataset == 'CXRAY':
             data_transform = transforms.Compose([
                 transforms.Resize((224, 224)),  # Standard size for most vision models
                 transforms.ToTensor(),
             ])
             ...
             #     transform=data_transform
             # )
             train_set = datasets.ImageFolder(os.path.join(os.path.join(data_dir, 'chest_xray'), 'train'),
                                              data_transform)
             
             img_size = 224  # Standard size for OCT images
             num_classes = 2  # OCT typically has 4 classes: CNV, DME, DRUSEN, and NORMAL
