...
parser.add_argument('-dataset', type=str, required=False,
                    default='cifar10',  # Changed this
                    choices=default_args.parser_choices['dataset'])
parser.add_argument('-poison_type', type=str, required=False,
                    choices=default_args.parser_choices['poison_type'],
                    default='WaNet')  # Changed this
parser.add_argument('-poison_rate', type=float, required=False,
                    choices=default_args.parser_choices['poison_rate'],
                    default=0.05)  # Changed this
parser.add_argument('-cover_rate', type=float, required=False,
                    choices=default_args.parser_choices['cover_rate'],
                    default=0.1)  # Changed this
parser.add_argument('-alpha', type=float, required=False,
                    default=0.15)  # Changed this
parser.add_argument('-test_alpha', type=float, required=False, 
                    default=0.2)  # Changed this from None to 0.2
parser.add_argument('-trigger', type=str, required=False, default=None)
parser.add_argument('-model_path', required=False, default='poisoned_train_set/cifar10/WaNet_0.050_cover=0.100_poison_seed=0/bad_expert_finetuned_full_base_aug_seed=2333.pt')
...
# Add WaNet specific arguments
parser.add_argument('-wanet_model_path', type=str, required=False, default='model/',
                  help='Path to the WaNet checkpoint file (.pth.tar)')
parser.add_argument('-s', type=float, default=0.5,
                  help='WaNet warping parameter')
parser.add_argument('-grid_rescale', type=float, default=1,
                  help='WaNet grid rescale parameter')
...
def load_wanet_model_and_parameters(model_path):
    """
    Load the WaNet model and extract necessary parameters from the checkpoint
    """
    # Load the checkpoint
    ...
    return model_state, identity_grid, noise_grid

def create_wanet_poison_transform(identity_grid, noise_grid, s=0.5, grid_rescale=1, input_height=256):
    """
    Create a poison transform function using WaNet parameters
    """
    def poison_transform(inputs):
        grid_temps = (identity_grid + s * noise_grid / input_height) * grid_rescale
        grid_temps = torch.clamp(grid_temps, -1, 1)
        
        # Apply grid sample
        poisoned = F.grid_sample(inputs, grid_temps.repeat(len(inputs), 1, 1, 1), align_corners=True)
        return poisoned
...
def main():
    ...
    # Load WaNet model and parameters
    model_state, identity_grid, noise_grid = load_wanet_model_and_parameters(args.wanet_model_path)
    
    # Move grids to device
    identity_grid = identity_grid.to(args.device)
    noise_grid = noise_grid.to(args.device)
    
    # Create model (make sure this matches your WaNet architecture)
    ...
    # Create poison transform
    poison_transform = create_wanet_poison_transform(
        identity_grid,
        noise_grid,
        s=args.s,
        grid_rescale=args.grid_rescale,
