...
def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt,attack = False):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        if attack:
            ...
            num_bd = round(bs * rate_bd)
            num_cross = round(num_bd * opt.cross_ratio)
...
def eval(
    ...
    for batch_idx, (inputs, targets) in enumerate(test_dl):
        with torch.no_grad():
            ...
            if attack:
                ...
                # print("grid_temps shape:", grid_temps.shape)
                # print("grid_temps range:", grid_temps.min().item(), grid_temps.max().item())
                # print("input shape:", inputs.shape)  # or data.shape
    ...
    print("Training Grids Stats:")
    print("Identity Grid - Mean: {:.6f}, Std: {:.6f}, Min: {:.6f}, Max: {:.6f}".format(
        identity_grid.mean().item(),
        identity_grid.std().item(),
        identity_grid.min().item(),
        identity_grid.max().item()
    ))
    print("Noise Grid - Mean: {:.6f}, Std: {:.6f}, Min: {:.6f}, Max: {:.6f}".format(
        noise_grid.mean().item(),
        noise_grid.std().item(),
        noise_grid.min().item(),
        noise_grid.max().item()
    ))
    ...
    if best_bd:
        ...
        print(f"Saved model to {opt.ckpt_path}")
...
def main():
    ...
    # opt.ckpt_path = 'classifier_checkpoint.pth'
