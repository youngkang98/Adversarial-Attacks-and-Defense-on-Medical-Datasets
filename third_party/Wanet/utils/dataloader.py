...
def get_dataloader(opt, train=True,trainOrTestData='Train', pretensor_transform=False):
    ...
    if opt.dataset == "gtsrb":
        ...
        root_path = str(config.get_data_path('ISIC2019'))
        test_root_path = str(config.get_data_path('ISIC2019_test'))
        if trainOrTestData == 'Train':
            ...
            dataset = CSVDataset(root=root_path, csv_file=csv_path, image_field='image', target_field='label',
                                 transform=transform, add_extension='.jpg',
                                 split=str(config.get_data_path('validation')) + str(opt.split_idx) + '.txt')
        elif trainOrTestData == 'Test':
            dataset=CSVDataset(root=root_path, csv_file=csv_path, image_field='image', target_field='label',
                              transform=transform, add_extension='.jpg',
                              # split=None)
                              split=str(config.get_data_path('train0.txt')).replace('train0.txt', 'test0.txt'))
        ...
        root_dir = str(config.get_data_path('COVID-Net'))+trainOrTestData
        ...
        trainFile = str(config.get_data_path('CXRAY-train.csv'))
        if trainOrTestData == 'Train':
