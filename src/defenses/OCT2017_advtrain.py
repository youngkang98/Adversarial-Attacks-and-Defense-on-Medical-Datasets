    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            if add_noise and noise_tensor is not None:
                ...
                if first_image:
                    make_adv_img(clean_image_for_psnr,noise_tensor,adv_img,f'{resultFolder}adv_img_att{eps_current[0]}eps{eps_current[1]}_{image_count}.jpg')
                    psnr(clean_image_np, adv_img_np,f'{resultFolder}psnr_att{eps_current[0]}_eps{eps_current[1]}.txt')
                    first_image = False
...
adversarialFile = str(config.get_data_path('OCT2017-train.csv'))
noiseFile = str(config.get_data_path('OCT2017-train-adv-0.1.csv'))
...
save_results_to_file(resultFolder + str(config.get_experiment_path('evaluation_result_after_adv_train.txt')),val_loss_no_noise,val_acc_no_noise, 0, 0, 0,targeted=False)
classificationReportFileName = resultFolder + str(config.get_experiment_path('classification_report_clean_after_advtrain.txt'))
