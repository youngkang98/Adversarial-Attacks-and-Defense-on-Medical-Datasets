...
##### Code to use ImageFolder ######

from tensorflow.keras.preprocessing.image import ImageDataGenerator
import pandas as pd

df = pd.read_csv(str(config.get_data_path('ISIC2019_train.csv')))
...
test_file = str(config.get_data_path('ISIC2019_test.csv'))
...
art_datagen = PyTorchDataGenerator(iterator=dataloader, size=data_length, batch_size=batch_size)
...
for epoch in test_epoch:
    ...
    trainer.fit_generator(art_datagen, validation_data=(x_test, y_test), nb_epochs=epoch)
    ...
    plot_confusion_matrix(np.argmax(y_test, axis=1), x_test_pred, "confusion_matrix_clean")
    generate_classification_report(np.argmax(y_test, axis=1), x_test_pred, str(config.get_experiment_path('classfication_report_clean.txt')))
    ...
    plot_confusion_matrix(np.argmax(y_test, axis=1), x_test_attack_pred_eps8s2, "confusion_matrix_eps8")
    generate_classification_report(np.argmax(y_test, axis=1), x_test_attack_pred_eps8s2, str(config.get_experiment_path('classfication_report_eps8.txt')))
...
# plot_confusion_matrix(np.argmax(y_test, axis=1), x_test_attack_pred_eps16s4, "confusion_matrix_eps16")
# generate_classification_report(np.argmax(y_test, axis=1), x_test_attack_pred_eps16s4, str(config.get_experiment_path('classfication_report_eps16.txt')))
