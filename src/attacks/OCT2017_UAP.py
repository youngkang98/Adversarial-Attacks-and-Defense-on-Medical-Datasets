...
testfile = str(config.get_data_path('OCT2017-test.csv'))
adversarialFile = str(config.get_data_path('OCT2017-train-adv-0.1.csv'))
...
checkpoint = torch.load(str(config.get_model_path('AdvTrain_OCT2017_epoch20_BS32.pth')),map_location ='cpu')
...
save_results_to_file(str(config.get_experiment_path(f'evaluation_result_after_adv_train.txt')),val_loss_no_noise,val_acc_no_noise, 0, 0, 0,targeted=False)
classificationReportFileName = str(config.get_experiment_path(f'classification_report_clean_after_advtrain.txt'))
...
for current_attack_eps in attack_eps:
    for current_eps in eps:
        for percentage in adv_percentages:
            ...
            results_filename = str(config.get_experiment_path(f'evaluation_results_{adv_image_count}_att{current_attack_eps}_eps{current_eps}.txt'))
            ...
            classificationReportFileName = str(config.get_experiment_path(f'classification_report_{adv_image_count}_att{current_attack_eps}_eps{current_eps}.txt'))
            generate_classification_report(true_labels, pred_labels, classificationReportFileName)
